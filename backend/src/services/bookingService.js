...
const getPreco = async (idAula) => {
    ...
    const aula = await bookingRepo.findById(idAula);
    if (!aula) throw criarErro('Aula não encontrada.', 404);

    return { preco: aula.Preco };
};

const isPrazoValido = async (idMarcacao) => {
    if (!idMarcacao) throw criarErro('IdMarcacao é obrigatório.', 400);
    ...
    const diferencaHoras = (dataAulaCompleta - agora) / (1000 * 60 * 60);

    return {
        valido: diferencaHoras >= 24,
        horasRestantes: Math.max(0, diferencaHoras).toFixed(1)
    };
};
